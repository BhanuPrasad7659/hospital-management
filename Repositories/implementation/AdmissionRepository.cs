...
namespace CogMediHospitalManagementSystem.Repositories.implementation
{
    public class AdmissionRepository : Repository<Admission>, IAdmissionRepository
    {
        ...
        public Admission AdmitPatient(int patientId, string ward, string bedNumber, int doctorId, string doctorName)
        {
            ...
            // NEW: Fetch the patient details from the database
            ...
            if (age <= 0)
            {
                throw new ArgumentException("Patient age must be greater than 0 to be admitted.");
            }

            var admission = new Admission
            {
                PatientId = patientId,
                PatientName = patient?.Name ?? "Unknown", // NEW: Assign Patient Name
                Age = age,                                // NEW: Assign Age
